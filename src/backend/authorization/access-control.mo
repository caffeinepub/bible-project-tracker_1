...
module {
  ...
  // All subsequent principals become regular users.
  public func initialize(state : AccessControlState, caller : Principal, adminToken : Text, userProvidedToken : Text) {
    if (caller.isAnonymous()) { return };
    switch (state.userRoles.get(caller)) {
      case (?_) {}; // Already registered, do nothing
      case (null) {
        // First user to register always becomes admin (no token needed)
        // Subsequent users can also claim admin with the correct token
        if (not state.adminAssigned) {
          state.userRoles.add(caller, #admin);
          state.adminAssigned := true;
        } else if (adminToken != "" and userProvidedToken == adminToken) {
          // Additional admins can be added via token
          state.userRoles.add(caller, #admin);
        } else {
